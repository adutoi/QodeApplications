...
void opPsi_2e(Double* op,            // tensor of matrix elements (integrals), assumed antisymmetrized
    ...
    BigInt  qsr_config[n_configint];
    BigInt pqsr_config[n_configint];

    int     cum_occ[n_orbs];    // the number of orbitals below a given index that are occupied (for phase calculations)
    int   r_cum_occ[n_orbs];    // the number of orbitals below a given index that are occupied (for phase calculations)
    int  sr_cum_occ[n_orbs];    // the number of orbitals below a given index that are occupied (for phase calculations)
    int qsr_cum_occ[n_orbs];    // the number of orbitals below a given index that are occupied (for phase calculations)

    #pragma omp parallel for private(occupied, empty, cum_occ, r_cum_occ, sr_cum_occ, qsr_cum_occ, r_config, sr_config, qsr_config, pqsr_config)
    for (PyInt n=0; n<n_configs; n++)
        {
        Double biggest = 0;                   // The biggest n-th component of all vectors being acted on
        for (int v=vec_0; v<vec_0+n_vecs; v++)
        ...
        if (biggest > thresh)    // all of this is skipped if the configuration has no significan coefficiencts
            ...
            for (int r_=0; r_<n_occ; r_++)
                ...
                for (int s_=r_+1; s_<n_occ; s_++)
                    ...
                    Q = s / n_bits;
                    R = s % n_bits;
                    int sr_permute = r_permute + r_cum_occ[n_orbs-1] - r_cum_occ[s];
                    memcpy(sr_config, r_config, n_bytes);
                    sr_config[Q] = sr_config[Q] ^ ((BigInt)1<<R);
                    memcpy(sr_cum_occ, r_cum_occ, n_bytes2);
                    for (int i=s; i<n_orbs; i++) {sr_cum_occ[i] -= 1;}
                    empty[n_emt+1] = s;
                    for (int q_=0; q_<n_emt+2; q_++)
                        {
                        int q = empty[q_];
                        Q = q / n_bits;
                        R = q % n_bits;
                        int qsr_permute = sr_permute + sr_cum_occ[n_orbs-1] - sr_cum_occ[q];
                        memcpy(qsr_config, sr_config, n_bytes);
                        qsr_config[Q] = qsr_config[Q] ^ ((BigInt)1<<R);
                        memcpy(qsr_cum_occ, sr_cum_occ, n_bytes2);
                        for (int i=q; i<n_orbs; i++) {qsr_cum_occ[i] += 1;}
                        for (int p_=q_+1; p_<n_emt+2; p_++)
                            {
                            int p = empty[p_];
                            // Interpret the component being looped over as the operator:
                            ...
                            if (fabs(op_pqrs)*biggest > thresh)    // cull all of the inner operations if matrix element not significant
                                ...
                                Q = p / n_bits;
                                R = p % n_bits;
                                memcpy(pqsr_config, qsr_config, n_bytes);
                                ...
                                if (m != -1)
                                    {
                                    int pqsr_permute = qsr_permute + qsr_cum_occ[n_orbs-1] - qsr_cum_occ[p];
                                    int phase = (pqsr_permute%2) ? -1 : 1;
                                    op_pqrs *= phase;
                                    for (int v=vec_0; v<vec_0+n_vecs; v++)
                                        {
