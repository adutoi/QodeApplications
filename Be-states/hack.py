...
for chg,states_chg in states.items():
    ...
    if num_states>0:
        ...
        ######################
        Zold = numpy.load("old-data/Z_{}e.npy".format(2-chg))
        Cold = numpy.load("old-data/configs_{}e-stable.npy".format(2-chg))
        print(Zold.shape, Cold.shape)
        for j,occupied in enumerate(Cold):
            i = field_op.find_index_by_occ(occupied, field_op.packed_configs(states_chg.configs))
            if i!=-1:
                print(occupied_to_str(occupied), "  {:018b}".format(states_chg.configs[i]) , "|", end="")
                w = 0
                for k,z in enumerate(Zold[j,:]):
                    zref = states_chg.coeffs[k][i]
                    print("  {: .1e}_{: .1e}".format(z, zref), end="")
                    w += z**2
                    states_chg.coeffs[k][i] = z
                print(" | {: .1e}".format(w))
            else:
                print(occupied_to_str(occupied), " "*20, "|", end="")
                w = 0
                for z in Zold[j,:]:
                    print("  {: .1e}         ".format(z), end="")
                    w += z**2
                print(" | {: .1e}".format(w))
        ######################
