...
def compress(rho_ij, op_string, bra_chg, ket_chg, i, j, compress, natural_orbs, antisymm_abstract):
    ...
    if c_count>0 and a_count>0 and compress[0]=="SVD":
        ...
        try:
            ret_rho = svd_decomposition(numpy.array(XR_tensor.raw(rho_ij), dtype=Double.numpy, order="C"), left_indices, right_indices, thresh=thresh, wrapper=XR_tensor.init)
        except: # numpy.linalg.LinAlgError:
            ret_rho = rho_ij
        rho_ij = ret_rho
    elif c_count+a_count>1 and compress[0]=="multi":
        rho_ij = multi_term(rho_ij, c_count, a_count, compress[1], XR_tensor.init)
    if natural_orbs is not None:
        p = 0
        for _ in range(c_count):
            indices_p = list(indices)
