...
// Checks whether a value is absent from an array of PyInts
int not_in(int a, PyInt* array, PyInt n)
    {
    int answer = 1;                          // start by assuming it is absent
    for (int i=0; i<n; i++)
        {
        if (a == array[i])  {answer = 0;}    // means it is not not_in the array (so it is)
        }
    return answer
    }
...
void resolve_recur(int      mode,             // OP_ACTION or COMPUTE_D (determines whether using Psi_L for storing new states or as bras)
                   ...
                   PyInt*   create_range,     // allowed values of the creation indices
                   PyInt    n_create_range,   // number of values in create_range (-1 means array not unused)
                   PyInt    n_annihil,        // number of annihilation operators at present level of recursion
                   PyInt*   annihil_range,    // allowed values of the annihilation indices
                   PyInt    n_annihil_range,  // number of values in annihil_range (-1 means array not unused)
                   ...
                   PyInt    tensor_override,  // boolean; in the special case of only one nonzero matrix element (see create_range and annihil_range), ...
                   Double   tens_over_val,    // ... this is it (for descending-order creation and annhilation strings)
    ...
    int    p_n;
    int*   orb_list;
    PyInt* restrict_range;      // **** DO NOT USE restrict_range WITH COMPUTE_D MODE (not-yet-implemented option) ****
    PyInt  n_restrict_range;
    if (n_annihil > 0)    // will loop over annihilation (occupied) index
        ...
        restrict_range   = annihil_range;      // the index needs to fall in annihil_range ...
        n_restrict_range = n_annihil_range;    // ... in order to continue
        }
        ...
        restrict_range   = create_range;       // the index needs to fall in create_range ...
        n_restrict_range = n_create_range;     // ... in order to continue
        }
    ...
    if (n_annihil + n_create > 1)    // recursive part (there is still >1 loop to go)
        ...
        for (int p_=p_0; p_<p_n; p_++)    // loop over the "counting" index for either occupieds or empties (starting from the given value; see below)
            ...
            int resolve_it = 1;      // proceed as originally planned, ...
            if (n_restrict_range >= 0)
                {
                if (not_in(p, restrict_range, n_restrict_range)) {resolve_it = 0;}    // ... unless the present index falls outside its respective range
                }
            if (resolve_it)
                {
                ...
                resolve_recur(mode, n_create, create_range, n_create_range, n_annihil, annihil_range, n_annihil_range, Psi_L, n_Psi_L, configs_L, n_configs_L, n_configint_L, Psi_R, n_Psi_R, p_config_R, config_idx_R, n_configint_R, tensors, tensor_override, tensor_over_val, n_orbs, occupied, n_occ, empty, n_emt, p_cum_occ, p_permute, op_idx+p*stride, stride*n_orbs, factor, q_0, thresh);
                }
            }
        }
        ...
        for (int p_=p_0; p_<p_n; p_++)    // final orbital loop (see above)
            {
            Double val = 0;    // dummy value becaused used in if condition even if not ever set (precedence should make this ok, but better be explicit)
            ...
            if (resolve_it)
                {
                if (tensor_override)  {val = factor * tensor_over_val;}                  // a convenience that user need not build an entire tensor just for this
                else                  {val = factor * tensors[0][p*stride + op_idx];}    // finish building tensor index (done inline with recursion above) and get integral from only tensor
                }
            if (resolve_it && fabs(val)>thresh)    // do nothing if the integral is too small (thresh considers also ket coefficient)
                {
    ...
    else if (mode == COMPUTE_D)    // bottom out option    **** DO NOT USE restrict_range WITH COMPUTE_D MODE (not-yet-implemented option) ****
        {
        for (int p_=p_0; p_<p_n; p_++)    // final orbital loop (see above)
            {
            int p = orb_list[p_];                // absolute index (see above)
...
void resolve(int      mode,             // OP_ACTION or COMPUTE_D (determines whether using Psi_L for storing new states or as bras)
    ...
    for (PyInt n=0; n<n_configs_R; n++)    // loop over configurations in ket basis
        ...
        if (biggest > thresh)    // do nothing if the configuration has no significant coefficients
            ...
            // This entire block of code is optional.  It only prevents computation when it can see ahead (inexpensively) that the result will be to
            // later decide not to act.  Even though operating on not all indices, with the exception of all-"c" field strings, this will lower the scaling
            // of the computation (in terms of number of orbitals, but unclear if that matters).  The reason we do not do this for c operators is because
            // we can also get nonzero when c and a indices match (but we are not yet at index level ... which is why what we do have is better).
            int resolve_it = 1;
            if (n_annihil_range >= 0)
                {
                for (int p=0; p<n_occ; p++)
                    {
                    if (not_in(occupied[p], annihil_range, n_annihil_range)) {resolve_it = 0;}
                    }
                }
            ...
            if (resolve_it)
                {
                resolve_recur(mode, n_create, create_range, n_create_range, n_annihil, annihil_range, n_annihil_range, Psi_L, n_Psi_L, configs_L, n_configs_L, n_configint_L, Psi_R, n_Psi_R, config, n, n_configint_R, tensors, tensor_override, tens_over_val, n_orbs, occupied, n_occ, empty, n_emt, cum_occ, permute, 0, 1, 1, 0, thresh/biggest);
                }
            }
        }
...
void op_Psi(PyInt    n_elec,        // electron order of the operator
    ...
    resolve(OP_ACTION, n_elec, create_range, n_create_range, n_elec, annihil_range, n_annihil_range, opPsi, n_Psi, configs, n_configs, n_configint, Psi, n_Psi, configs, n_configs, n_configint, &op, tensor_override, tens_over_val, n_orbs, permute, thresh, n_threads);
    return;
    }
...
void densities(PyInt    n_create,           // number of creation operators
               ...
               PyInt    n_threads)          // number of threads to spread the work over
    {
    PyInt* dummy;    // as of now we do not use restricted ranges for densities (but we could!)
    // call the generic driver in compute-densities mode
    resolve(COMPUTE_D, n_create, dummy, -1, n_annihil, dummy, -1, bras, n_bras, configs_bra, n_configs_bra, n_configint_bra, kets, n_kets, configs_ket, n_configs_ket, n_configint_ket, rho, 0, 0., n_orbs, 0, thresh, n_threads);
    return;
    }
