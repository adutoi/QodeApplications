...
void resolve_recur(int      mode,             // OP_ACTION or COMPUTE_D (determines whether using Psi_L for storing new states or as bras)
    ...
    if (n_annihil + n_create > 1)    // recursive part (there is still >1 loop to go)
        ...
        for (int p_=p_0; p_<p_n; p_++)    // loop over the "counting" index for either occupieds or empties (starting from the given value; see below)
            ...
            resolve_recur(mode, n_create, n_annihil, Psi_L, n_Psi_L, configs_L, n_configs_L, n_configint_L, Psi_R, n_Psi_R, p_config_R, config_idx_R, n_configint_R, tensors, n_orbs, occupied, n_occ, empty, n_emt, p_cum_occ, p_permute, op_idx+p*stride, stride*n_orbs, factor, q_0, thresh, key_L, len_key_L, key_R, len_key_R, core_mask);
            }
        }
        ...
        for (int p_=p_0; p_<p_n; p_++)    // final orbital loop (see above)
            ...
            p_config_R[Q] = p_config_R[Q] ^ ((BigInt)1<<r);    // ... above
            int core_occ = 1;
            for (Q=0; Q<n_configint_R; Q++)
                {
                if ((core_mask[Q] & p_config_R[Q]) != core_mask[Q])  {core_occ = 0;}
                }
            if (core_occ)
                {
                //PyInt config_idx_L = bisect_search(p_config_R, configs_L, n_configint_L, 0, n_configs_L-1);    // EXPENSIVE! -- find left-basis index of full string on right config
...
void resolve(int      mode,             // OP_ACTION or COMPUTE_D (determines whether using Psi_L for storing new states or as bras)
    ...
    BigInt core_mask[n_configint_R];
    for (int i=0; i<n_configint_R; i++)  {core_mask[i] = 0;}
    for (int p_=0; p_<n_core; p_++)
        {
        int p = core[p_];
        int Q = p / n_bits;
        int r = p % n_bits;
        core_mask[Q] = core_mask[Q] ^ ((BigInt)1<<r);
        }
    ...
    for (PyInt n=0; n<n_configs_R; n++)    // loop over configurations in ket basis
        ...
        if (biggest > thresh)    // do nothing if the configuration has no significant coefficients
            ...
            resolve_recur(mode, n_create, n_annihil, Psi_L, n_Psi_L, configs_L, n_configs_L, n_configint_L, Psi_R, n_Psi_R, config, n, n_configint_R, tensors, n_orbs, occupied, n_occ, empty, n_emt, cum_occ, permute, 0, 1, 1, 0, thresh/biggest, key_L, len_key_L, key_R, len_key_R, core_mask);
            }
        }
...
void op_Psi(PyInt    n_elec,        // electron order of the operator
    ...
    resolve(OP_ACTION, n_elec, n_elec, opPsi, n_Psi, configs, n_configs, n_configint, Psi, n_Psi, configs, n_configs, n_configint, &op, n_orbs, permute, thresh, n_threads, core, n_core);
    return;
    }
...
void densities(PyInt    n_create,           // number of creation operators
    ...
    resolve(COMPUTE_D, n_create, n_annihil, bras, n_bras, configs_bra, n_configs_bra, n_configint_bra, kets, n_kets, configs_ket, n_configs_ket, n_configint_ket, rho, n_orbs, 0, thresh, n_threads, core, n_core);
    return;
    }
