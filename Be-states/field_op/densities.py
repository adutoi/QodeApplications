...
def build_tensors(states, n_orbs, n_elec_0, thresh=1e-10, n_threads=1, core=[]):
    print("CORE", core)
    densities = {}
    ...
    op_strings = {2:["aa", "caaa"], 1:["a", "caa"], 0:["ca", "ccaa"], -1:["c", "cca"], -2:["cc", "ccca"]}
        ...
        for ket_chg in states:
            ...
            if chg_diff in op_strings:
                for op_string in op_strings[chg_diff]:
                    ...
                    print("CORE", core)
                    rho = field_op.build_densities(op_string, n_orbs, bra_coeffs, ket_coeffs, bra_configs, ket_configs, thresh, n_threads, core)
