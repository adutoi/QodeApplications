...
def integrals(fragments, n_elec_frag, n_core_frag):
	...
	lilC = inv_rt(core_S)
	bigC = numpy.identity(2*n_spatial_orb)

	for i in range(n_core):
		for j in range(n_core):
			bigC[core_orbs[i],core_orbs[j]] = lilC[i,j]

	newS = bigC.T @ sMO_S @ bigC

	for i in range(2*n_spatial_orb):
		if i not in core_orbs:
			Vi = numpy.array(bigC[:,i])
			for j in core_orbs:
				Vj = bigC[:,j]
				Sji = newS[j,i]
				Vi -= Vj * Sji
	...
	sMO_h, sMO_V = MO_transform(sMO_h, sMO_V, bigC)
	...
	bigCinv = precise_numpy_inverse(bigC)
	sMO_h, sMO_V = MO_transform(sMO_h, sMO_V, bigCinv)
