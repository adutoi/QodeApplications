...
numpy.set_printoptions(linewidth=1000, precision=2)
...
def MO_transform(h, V, right, left=None):
	if left is None:  left = right.T
	h = left @ h @ right
	for _ in range(2):  V = numpy.tensordot(V, left.T, axes=([0],[0]))       # cycle through the tensor axes (this assumes everything is real)
	for _ in range(2):  V = numpy.tensordot(V,  right, axes=([0],[0]))       # cycle through the tensor axes (this assumes everything is real)
	return h, V
...
def integrals(fragments, n_elec_frag, n_core_frag):
	...
	SemiMOints = spin_orb_integrals(SemiMOints)

	sMO_S = unblock_2(    fragments, SemiMOints.S, spin_orbs=True)
	sMO_h = unblock_2(    fragments, SemiMOints.T, spin_orbs=True)
	sMO_U = unblock_last2(fragments, SemiMOints.U, spin_orbs=True)
	sMO_V = unblock_4(    fragments, SemiMOints.V, spin_orbs=True)
	for _,U in sMO_U.items():  sMO_h += U

	n_frag = len(fragments)
	n_spat_orb_frag  = sMO_h.shape[0] // (2*n_frag)
	...
	for i in range(n_core):
		for j in range(n_core):
			core_S[i,j] = sMO_S[core_orbs[i],core_orbs[j]]
	core_prj = precise_numpy_inverse(core_S)

	left  = precise_numpy_inverse(sMO_S)
	for i in range(n_core):
		for j in range(2*n_spatial_orb):
			left[core_orbs[i],j] = 0
		for j in range(n_core):
			left[core_orbs[i],core_orbs[j]] = core_prj[i,j]

	newS = left @ sMO_S
	right = numpy.identity(2*n_spatial_orb)
	for i in range(2*n_spatial_orb):
		if i not in core_orbs:
			for j in core_orbs:
				right[j,i] = -newS[j,i]

	print(left @ sMO_S @ right)

	bsMO_h, bsMO_V = MO_transform(sMO_h, sMO_V, right, left)
		...
		for p in range(2*n_spatial_orb):
			for q in range(2*n_spatial_orb):
				for r in range(2*n_spatial_orb):
					if p!=c and q!=c:  sMO_V[p,q,r,c] = 0
					if p!=c and q!=c:  sMO_V[p,q,c,r] = 0
					if p!=c and q!=c:  sMO_V[r,c,p,q] = 0
					if p!=c and q!=c:  sMO_V[c,r,p,q] = 0
	"""

	bsMO = empty()
	bsMO.h, bsMO.V = bsMO_h, bsMO_V
