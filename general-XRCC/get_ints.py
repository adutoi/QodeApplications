...
from qode.atoms.integrals.fragments import AO_integrals, semiMO_integrals, bra_transformed, ket_transformed, spin_orb_integrals, Nuc_repulsion, as_raw_mat, as_frag_blocked_mat, zeros2, Id, mat_as_rows, mat_as_columns, space_traits, add, subtract, mat_mul, cached
...
def get_ints(fragments):
    ...
    Left, Right = direct_CoreSinv(fragments, SemiMO_ints.S)
    BiSemiMO_ints_half = bra_transformed(Left,  SemiMO_ints, cache=True)
    BiSemiMO_ints      = ket_transformed(Right, BiSemiMO_ints_half)	# no need to cache because each block only called once

    BiSemiMO_spin_ints = spin_orb_integrals(BiSemiMO_ints, rule_wrappers=[Double_array])     # no need to cache because each block only called once by contraction code
    return BiSemiMO_spin_ints, Nuc_repulsion(fragments)
