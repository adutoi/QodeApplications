...
def compress(rho_ij, op_string, bra_chg, ket_chg, i, j, options, natural_orbs, antisymm_abstract):
    ...
    if c_count>0 and a_count>0 and options.method=="SVD":
        ...
        rho_ij = svd_decomposition(numpy.array(XR_tensor.raw(rho_ij), dtype=Double.numpy, order="C"), left_indices, right_indices, thresh=thresh, wrapper=XR_tensor.init)
    if natural_orbs is not None:
        p = 0
        for _ in range(c_count):
            indices_p = list(indices)
