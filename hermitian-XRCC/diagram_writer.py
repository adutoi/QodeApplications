...
#    a. most important is thing about shared index instances
#    b. ordering of terms is from outside, ordering of intergrals is inside
#    c. fragment labels is from outside
#    d. philosphy behind external functions (that access ._ functions) and those that change object in place, but only superficially
# 2. search below for NEED TO GENERALIZE, bc would be a bug for !=2 frags, but also see below
# 3. there is some inconsistency about fragment permutations when all integrals on one frag.  How to solve:
#    a. .exp_val() should take fragment arguments, just to be consistent that this is not an "internal" decision
#    b. frag_factored() should similarly take fragment arguments and issue a kroenecker as a kind of integral if that fragment not operated on
#    c. then everything else will slide through.  We will still get the + P12, but at least it will be "real" and sensible to
#       an outsider. Making clear that it is a single-fragment term (the P12 just switches the nontrivial part and the kroeneker)
# switch from n_{i_2} to n_{j_1} convention and start numerically vetting autogenerated code (maybe swap orders of these)
#
# Found elswhere later:
# v1111 has permutations
# Put out code that is closer to reality, including factorial
# Include permutations and charges
# Order by CT character?
