...
import pickle
import numpy
from build_density_tensors_hacked import build_density_tensors
...
def _load_states(digit, n_states, states_location):
    ############################################
    states_file  = "atomic_states/states/16-115-550/{}/U_{}e.npy".format(states_location, digit)
    ############################################
...
#states   = "thresh=1e-6/4.5"
states   = "dummy-determinants"
n_states = ("0", "10", "10", "10", "0")
Be = monomer_data(None)
Be.load_states(states, n_states)
#pickle.dump(Be.rho, open("Be_rho.pkl","wb"))
pickle.dump(Be.rho, open("Be_rho_HFdet.pkl","wb"))
