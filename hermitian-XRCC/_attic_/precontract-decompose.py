...
from qode.math.tensornet     import raw, evaluate, tl_tensor
...
def precontract(densities, integrals, timings, thresh=1e-6):
        ...
        def contract_rho_int(*indices):
            ...
            def contract_rho_int_m(chg_i,chg_j):
                def contract_rho_int_m_chgs(i,j):
                    ...
                    result = numpy.array(raw(rho(*rho_indices) @ ints(*int_indices)))
                    result = tens_wrap(result)
                    #result = svd_decomposition(result, rho_free, int_free, thresh=thresh, wrapper=tens_wrap)
