...
def compute_det(first_idx, second_idx, num_valence_elec_atom, Be_fci_states, Be2_fci_states, S_Atomic_overlap):
	...
	C_det = numpy.zeros((dimension,dimension), dtype=PyC.Double.numpy)
...
def compute_column(*args):
	...
	results_vec = numpy.matrix(numpy.zeros((1,Be_n_state), dtype=PyC.Double.numpy))
...
def compute_transform_matrix(C_Be_long, 
	...
	Be_fci_states = numpy.array(frozen_core_fci_states(num_spin_orbs_atom, num_elec_atom, num_core_elec_atom)[1])
	Be2_fci_states = numpy.array(frozen_core_fci_states(num_spin_orbs_dimer, num_core_elec_dimer + num_valence_elec_atom, num_core_elec_dimer)[1])
	...
	C_Mol_overlap = numpy.zeros((Be2_n_state, Be_n_state), dtype=PyC.Double.numpy)
	...
	#myPool.terminate()
	results = [compute_column([i, num_valence_elec_atom, Be_n_state, Be_fci_states, Be2_fci_states, S_Atomic_overlap]) for i in range(Be2_n_state)]
	...
	# numpy.save( "integrals/S_Be_Be2_{}.npy", S_Atomic_overlap)
	# print("OneBody to TwoBody Transformation Matrix Done.")
	return numpy.matrix(C_Mol_overlap), S_Atomic_overlap
...
def getBasisOverlapMatrix( C_scf_atom,     C_scf_dimer,  S_dimer, \
	...
	# C_Be_long
	C_Be_long = numpy.matrix(numpy.zeros((num_spin_orbs_dimer,num_spin_orbs_atom), dtype=PyC.Double.numpy))
	...
	# numpy.save("integrals/S_Be_Be2_High.npy", S_Atomic_overlap_High)
	...
	# numpy.save("integrals/S_Be_Be2_Low.npy", S_Atomic_overlap_Low)
