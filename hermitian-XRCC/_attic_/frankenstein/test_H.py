...
for m,frag in enumerate(BeN):  frag.load_states(states, integrals.V[m,m,m,m], n_states)


for m1 in (0,1):
    for m2 in (0,1):
        numpy.save("test-data-{}/S_{}_{}".format(sys.argv[2],m1,m2), integrals.S[m1,m2])
        numpy.save("test-data-{}/T_{}_{}".format(sys.argv[2],m1,m2), integrals.T[m1,m2])
        for m3 in (0,1):
            numpy.save("test-data-{}/U_{}_{}_{}".format(sys.argv[2],m1,m2,m3), integrals.U[m1,m2,m3])
            for m4 in (0,1):
                numpy.save("test-data-{}/V_{}_{}_{}_{}".format(sys.argv[2],m1,m2,m3,m4), integrals.V[m1,m2,m3,m4])
...
if __name__ == '__main__':
    ...
    E, T = excitonic.ccsd((H1,H2), out, resources)
    out.log("\nTotal Excitonic CCSD Energy = ", E)

    # Compute the XR2-FCI energy
    out, resources = output(log=textlog(echo=True)), parallel.resources(1)
    numpy.save("test-data-{}/H1_0".format(sys.argv[2]), H1[0])
    numpy.save("test-data-{}/H1_1".format(sys.argv[2]), H1[1])
    numpy.save("test-data-{}/H2_0_1".format(sys.argv[2]), H2[0][1])
    E = excitonic.fci((H1,H2), out)
