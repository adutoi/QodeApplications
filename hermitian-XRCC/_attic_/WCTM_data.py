...
Srefs = {n_elec:numpy.load("atomic_states/states/16-115-550/load=states:16-115-550:thresh=1e-6:4.5:u.pickle/{}/S-{}.npy".format(displacement,n_elec)) for n_elec in [6,7,8,9,10]}

big_dim = sum(Stest[n_elec].shape[0] for n_elec in [6,7,8,9,10])
BigS = numpy.zeros((big_dim,big_dim))
BigS.fill(100)    # later interpreted as +1e-100 (as good as actual zero for our plots)

norms = [numpy.max(Srefs[n_elec]) for n_elec in [6,7,8,9,10]]
print("norms: ", norms)
norm = max(norms)
...
for n_elec in [6,7,8,9,10]:
    ...
    print("   max deviation of S-Sref:  ", numpy.max(numpy.abs(Sdiff)))
    ...
    for i in range(dim):
        ...
        for j in range(dim):
            ...
            BigS[I,J] = math.copysign(math.log10(abs(Sdiff[i][j]/norm)), Sdiff[i][j])     # relies on numbers all smaller than 1
        dumpS.write("\n")
        dumpd.write("\n")
    dumpS.close()
    dumpd.close()
...
colormap = {"cmap":"twilight", "vmin":-6, "vmax":6}
# for rough work with colorbar legend
im = plt.matshow(BigS, **colormap)
plt.colorbar(im)
plt.savefig("viz-d-sampled.png", dpi=300)
plt.close()
# for pixel-precise image
plt.imsave("viz-d.png", BigS, **colormap)
plt.close()


###

body1_ref = numpy.load("_attic_/reference/test-data-4.5/H1_0.npy")
