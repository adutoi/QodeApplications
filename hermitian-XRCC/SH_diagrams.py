...
def _parameters1(densities, integrals, subsystem, charges):
    ...
    if data.Dchg_1==0:
        data.ca_1    = densities[m1]['ca'  ][chg_i1,chg_j1]
        data.ccaa_1  = densities[m1]['ccaa'][chg_i1,chg_j1]
    if data.Dchg_1==-1:
        data.c_1     = densities[m1]['c'   ][chg_i1,chg_j1]
        data.cca_1   = densities[m1]['cca' ][chg_i1,chg_j1]
    if data.Dchg_1==+1:
        data.a_1     = densities[m1]['a'   ][chg_i1,chg_j1]
        data.caa_1   = densities[m1]['caa' ][chg_i1,chg_j1]
    if data.Dchg_1==-2:
        data.cc_1    = densities[m1]['cc'  ][chg_i1,chg_j1]
    if data.Dchg_1==+2:
        data.aa_1    = densities[m1]['aa'  ][chg_i1,chg_j1]
    data.T_11 = T[m1,m1]
    data.U_1_11 = U[m1,m1,m1]
    data.V_1111 = V[m1,m1,m1,m1]
    return data
...
def _parameters2(densities, integrals, subsystem, charges, permutation):
    ...
    if data.Dchg_1==+2:
        data.aa_1    = densities[m1]['aa'  ][chg_i1,chg_j1]
    if data.Dchg_2==0:
        data.ca_2    = densities[m2]['ca'  ][chg_i2,chg_j2]
        data.ccaa_2  = densities[m2]['ccaa'][chg_i2,chg_j2]
    if data.Dchg_2==-1:
        data.c_2     = densities[m2]['c'   ][chg_i2,chg_j2]
        data.cca_2   = densities[m2]['cca' ][chg_i2,chg_j2]
    if data.Dchg_2==+1:
        data.a_2     = densities[m2]['a'   ][chg_i2,chg_j2]
        data.caa_2   = densities[m2]['caa' ][chg_i2,chg_j2]
    if data.Dchg_2==-2:
        data.cc_2    = densities[m2]['cc'  ][chg_i2,chg_j2]
    if data.Dchg_2==+2:
        data.aa_2    = densities[m2]['aa'  ][chg_i2,chg_j2]
    data.S_12 = S[m1,m2]
    data.S_21 = S[m2,m1]
    data.T_11 = T[m1,m1]
    data.T_12 = T[m1,m2]
    ...
    data.T_22 = T[m2,m2]
    data.U_1_11 = U[m1,m1,m1]
    data.U_1_12 = U[m1,m1,m2]
    data.U_1_21 = U[m1,m2,m1]
    data.U_1_22 = U[m1,m2,m2]
    data.U_2_11 = U[m2,m1,m1]
    data.U_2_12 = U[m2,m1,m2]
    data.U_2_21 = U[m2,m2,m1]
    data.U_2_22 = U[m2,m2,m2]
    data.V_1111 = V[m1,m1,m1,m1]
    data.V_1112 = V[m1,m1,m1,m2]
    data.V_1121 = V[m1,m1,m2,m1]
    data.V_1122 = V[m1,m1,m2,m2]
    data.V_1211 = V[m1,m2,m1,m1]
    data.V_1212 = V[m1,m2,m1,m2]
    data.V_1221 = V[m1,m2,m2,m1]
    data.V_1222 = V[m1,m2,m2,m2]
    data.V_2111 = V[m2,m1,m1,m1]
    data.V_2112 = V[m2,m1,m1,m2]
    data.V_2121 = V[m2,m1,m2,m1]
    data.V_2122 = V[m2,m1,m2,m2]
    data.V_2211 = V[m2,m2,m1,m1]
    data.V_2212 = V[m2,m2,m1,m2]
    data.V_2221 = V[m2,m2,m2,m1]
    data.V_2222 = V[m2,m2,m2,m2]
    return data
...
class body_1(object):
    ...
    def order1(densities, integrals, subsystem, charges):
        ...
        if X.Dchg_1==0:
            prefactor = 1
            def diagram(i1,j1):
                return prefactor * numpy.tensordot((X.T_11+X.U_1_11), X.ca_1[i1][j1], axes=([0,1],[0,1]))
            return [(diagram, (0,))]
        else:
            return [(None, None)]
    ...
    def order2(densities, integrals, subsystem, charges):
        ...
        if X.Dchg_1==0:
            prefactor = 1
            def diagram(i1,j1):
                return prefactor * numpy.tensordot(X.V_1111, X.ccaa_1[i1][j1], axes=([0,1,2,3],[0,1,3,2]))
            return [(diagram, (0,))]
        else:
            return [(None, None)]
...
class body_2(object):
    ...
    def _order1_CT0(densities, integrals, subsystem, charges, permutation):
        ...
        if X.Dchg_1==0 and X.Dchg_2==0:
            ...
            def diagram(i1,i2,j1,j2):
                if j1==j2:
                    return prefactor * numpy.tensordot(X.U_2_11, X.ca1[i1][j1], axes=([0,1],[0,1]))
                else:
    ...
    def _order1_CT1(densities, integrals, subsystem, charges, permutation):
        ...
        if X.Dchg_1==-1 and X.Dchg_2==+1:
            ...
            def diagram(i1,i2,j1,j2):
                partial =          numpy.tensordot((X.T_12 + X.U_1_12 + X.U_2_12), X.c1[i1][j1], axes=([0],[0]))
    ...
    def order2_CT0(densities, integrals, subsystem, charges):
        ...
        if X.Dchg_1==0 and X.Dchg_2==0:
            prefactor = 1
            def diagram(i1,i2,j1,j2):
                partial =          numpy.tensordot((V_1221-), axes=([0],[0]))
                return prefactor * numpy.tensordot(partial,       X.a2[i2][j2],                  axes=([0],[0]))
            return [(diagram, (0,1))]
        else:
            return [(None, None)]
